# -*- coding: utf-8 -*-
"""
Created on Sun Nov  2 18:19:49 2025
...
def f(x):
    x1 = x[0]
    x2 = x[1]
    return 3 + (x1 -   1.5*x2)**2 + (x2-2)**2

def gradient(x):
    x1 = x[0]
    x2 = x[1]
    gk = np.array([2*x1 - 3*x2,-3*x1 + 6.5*x2 - 4 ])
    return gk 
...
while all([C1,C2,C3,C4]):
    ...
    print('k:',k,'s:',s,'f(x):',f(x),'||gradient(x)||',np.linalg.norm(gradient(x)))
    C1 = k<N
...
plt.xlabel('x1')
plt.ylabel('x2')
plt.title('Steepest Descend Algoritması',fontstyle='italic')
